(function() {
  'use strict';
  pulse.controllers.controller('TimelapseCtrl', function($scope, $ionicSlideBoxDelegate, $ionicLoading, $rootScope, $ionicPlatform, $platform, $ionicSideMenuDelegate, $interval, $timeout, $timelapse, $transmit, $preset, $views, $device, $histogram, $camSettings, $config, $photo, $stateParams, $window, btClassic) {
    var vm = this;
    vm.processing = false;
    var buttonInterval;
    ...
    $rootScope.$on('startTimelapseUi', (event, device) => {
    ...
    $rootScope.$on('timelapseFinished', (event, data) => {
      var connectedPulses = $device.getConnectedDevices();
      if (connectedPulses.length < 2){
        if (data.deviceId == $stateParams.deviceId && $timelapse.timelapses[dId].settings.isActive){
          var modalData = {
    ...
    $rootScope.$on('timelapseReconnect', (event, data) => {
    ...
    vm.startTl = function() {
      ...
      if (device && device.metaData && device.metaData.cameraConnected) {
        ...
        if (!$camSettings.hasGoodShutterSetting(device, $timelapse.timelapses[dId].settings.interval)) {
          modalData = {
            text: 'We found a problem! Your shutter speed of ' + (device.metaData.camSettings.shutterOptions[device.metaData.camSettings.activeShutterIndex]).value + ' cannot be slower than your timelapse interval!',
            onButtonClick: function() {
          ...
          $rootScope.$broadcast('openModal', modalData);
          return;
        } else if (parseInt($timelapse.timelapses[dId].settings.interval) > ((parseInt($timelapse.timelapses[dId].settings.duration.hours) * 60 * 60) + (parseInt($timelapse.timelapses[dId].settings.duration.minutes) * 60))) {
          modalData = {
            text: 'We found a problem! Your interval cannot be longer than the length of your timelapse.',
            onButtonClick: function() {
        ...
        if ($timelapse.timelapses[dId].settings.activeDelay) {
          vm.tlMax = ((parseInt($timelapse.timelapses[dId].settings.delay.hours) * 60) + parseInt($timelapse.timelapses[dId].settings.delay.minutes)) * 60;
          countdown = $interval(()=>{
            vm.countDown++;
          }, 1000, vm.tlMax);
          timerIncrement = 1000;
      ...
      $ionicPlatform.on('resume', function(event) {
        ...
        if (device){
          if ($timelapse.timelapses[device.id].settings.isActive) {
            $timelapse.start(device, false, true).then(() => {
    ...
    vm.getDelaySeconds = function(){
      return ((parseInt($timelapse.timelapses[dId].settings.delay.hours) * 60) + parseInt($timelapse.timelapses[dId].settings.delay.minutes)) * 60;
    };

    function handleImageRender(){
      var currentPhoto = $timelapse.timelapses[dId].settings.enumeratingTl.photos;
      ...
      if (camSettings && camSettings.shutter){
        shutterWait = $views.getMillisecondsFromShutter(camSettings.shutter.value);
      }
      else{
        shutterWait = 50;
      }
      ...
      if ((($timelapse.timelapses[dId].settings.interval * 1000) - shutterWait) < minInterval){
      ...
      $timeout(() => {
        ...
        $photo.getThumb(device).then((result) =>{
        ...
        var currentPhoto = $timelapse.timelapses[dId].settings.enumeratingTl.photos;
        vm.currentPhoto = currentPhoto;
      }, ((parseInt($timelapse.timelapses[dId].settings.enumeratingTl.interval)) * 1000) + 1000);
    ...
    vm.toggleHistogram = function() {
      ...
      if (!vm.histogramItems || !vm.histogramItems.data || !vm.isBtClassicConnected()) {
        vm.histogram = false;
    ...
    vm.handleToggle = function(enabled){
      ...
      if (enabled){
        ...
        btClassic.isConnected(device.metaData.macAddress).then((result) =>{
          ...
          btClassic.connect(device.metaData.macAddress, device, false, true).then((result)=>{
