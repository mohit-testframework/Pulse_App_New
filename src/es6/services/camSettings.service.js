(function() {
  ...
  pulse.services.factory('$camSettings', function($config, $transmit, $device, $views) {
    ...
    return {
       ...
       * @return {null}
       */
      updateSetting: function(device, key, newIndex, updateActiveIndex = true) {
        var newVal;
        var byteArray;
        if (key == 'shutter') {
      ...
      findIndexForSettingValue: function(value, valueArray){
        var index;
        if (!valueArray.length){
          return;
        }
        _.forEach(valueArray, (arrayItem, $index)=>{
      ...
      getShutterFromIndex: function(index){
        var device = $device.getSelectedDevice();
        if (!device){
          return;
        }
      ...
      hasGoodShutterSetting: function(device, interval){
