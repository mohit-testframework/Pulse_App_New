  ...
  pulse.services.factory('$firmware', function($q, $cordovaFileTransfer, $cordovaNativeStorage, $cordovaFile, $transmit, $timeout, $views, $config, BLE, $platform, $location, $rootScope, btClassic, $ionicLoading) {
    ...
    return {
      ...
      syncMenuCompatability: function() {
        var deferred = $q.defer();
        this.getCompatability().then((compatabilityList) => {
      ...
      getMostRecentFirmware: function() {
        ...
        if (!window.cordova) {
          ...
          $cordovaFileTransfer.download(url, targetPath, options, trustHosts).then(function(result) {
            ...
            console.log('Firmware binary retrieval failed. Trying local fallback');
            var localPath;
            if ($platform.isAndroid()) {
            ...
            $cordovaFileTransfer.download(localPath, targetPath, options, trustHosts).then((result) => {
      ...
      readUpdateFirmware: function() {
        var deferred = $q.defer();
        this.getMostRecentFirmware().then((firmwareFile) => {
          this.readFirmwareFile(firmwareFile).then((arrayBuffer) => {
      ...
      readFirmwareFile: function(firmwareFile) {
        ...
        $cordovaFile.readAsArrayBuffer(path, firmwareFile.name).then((arrayBuffer) => {
          deferred.resolve(arrayBuffer);
        }, (error) => {
          ...
          var scanForBootloader = function() {
            ...
            $timeout(() => {
              ...
              BLE.stopScan().then(() => {
                ...
                ble.startScan(scanArray, (peripheral) => {
      ...
      update: function(dataArray, device) {
        ...
        var saveYourFOTA = function() {
          ...
          ble.read(device.id, $config.bootloader.BL_SERVICE, $config.bootloader.CMD_CHANNEL, bootloaderRX, (error) => {
        ...
        var fwCommandTransmit = function(cmd) {
          ble.write(device.id, $config.bootloader.BL_SERVICE, $config.bootloader.CMD_CHANNEL, $views.numToArray2(cmd), (res) => {}, tranError);
        };
        ...
        var fwDataTransmit = function(packetNum) {
          ...
          var i;
          if (packetNum == 0xFFFF) { //check if we're sending the erase packet

            console.log("Loading erase command into packet.");
            for (i = 0; i < 4; i++) {
              array[i + 2] = $views.decToHex(erase_pkt[i]); //move over the erase packet data
            }
          } else if (packetNum === 0) { //check if we're sending the first write packet
          ...
          } else if (((packetNum - 1) * BOOTLOADER_PACKET_SIZE) <= dataArray.length) { //if we're sending a regular data hunk
          ...
          ble.write(device.id, $config.bootloader.BL_SERVICE, $config.bootloader.DATA_CHANNEL, array.buffer, (res) => {}, tranError);
        };
        ...
        var kickOff = function() {
          ...
          ble.startNotification(device.id, $config.bootloader.BL_SERVICE, $config.bootloader.CMD_CHANNEL, bootloaderRX, (error) => {});
