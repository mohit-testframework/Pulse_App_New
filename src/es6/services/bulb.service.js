(function() {
  ...
  pulse.services.factory('$bulb', function($timeout, $interval, $q, $transmit, $device, $camSettings, $rootScope, $views) {
    ...
    return {
      ...
      startBulb: function() {
        ...
        if (!settings.isTimed){
          ...
          timer = $interval(() => {
            ...
            duration.manual.seconds = $views.getSeconds(this.time);
            duration.manual.minutes = $views.getMinutes(this.time);
          }, 1000);
      ...
      startInterval: function(seconds, active = false){
