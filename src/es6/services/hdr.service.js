(function() {
  ...
  pulse.services.factory('$hdr', function($transmit, $device, $views, $camSettings, $q, $timeout, $interval) {
    ...
    return {
      ...
      hdrCapture: function(device, deferred = $q.defer()) {
        ...
        if (settings.iterator < settings.numPhotos && !settings.forceStop) {
          ...
          settings.hdrPromise = $timeout(()=>{
            this.forceCapture(device);
          }, delay);
        } else {
      ...
      forceCapture: function(device){
        console.log('forcing capture');
        this.hdrCapture(device);
      },

      capture: function(device){
      ...
      prepareHdrData: function(device) {
