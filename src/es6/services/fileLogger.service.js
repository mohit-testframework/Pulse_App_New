  ...
  pulse.services.factory('$fileLogger', function($rootScope, $q, $cordovaFile, $filter, $window, $timeout) {
    ...
    function writeLog(message) {
      ...
      if (isBrowser()) {
        ...
        $cordovaFile.checkFile(logLoc, storageFilename).then(
          () => {
            // writeExistingFile(path, fileName, text)
            $cordovaFile.writeExistingFile(logLoc, storageFilename, message).then(
              () => {
                q.resolve();
              },
              (error) => {
          ...
          () => {
            ...
            $cordovaFile.writeFile(logLoc, storageFilename, message, true).then(
              () => {
                q.resolve();
              },
              (error) => {
    ...
    function getLogfile() {
      ...
      if (isBrowser()) {
        ...
        var logLoc = getStorageLoc();
        $cordovaFile.readAsText(logLoc, storageFilename).then(
          (result) => {
            q.resolve(result);
          },
          (error) => {
    ...
    function checkFile() {
      ...
      if (isBrowser()) {
        ...
        $cordovaFile.checkFile(logLoc, storageFilename).then((fileEntry) => {
