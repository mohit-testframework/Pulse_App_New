(function() {
  ...
  pulse.services.factory('$device', function($cordovaNativeStorage, $q, $timeout, $views, $config, $transmit, $rootScope, $cordovaFile, $platform, $fileSystem, $fileLogger, $ionicNativeTransitions, btClassic, $ionicHistory, BLE, $location, $ionicLoading, $firmware, $cordovaVibration) {
    ...
    function convertThumb(data, pulse) {
      ...
      $fileSystem.saveThumb(path, fileName, blob).then((success) => {
        var data = {
          hasThumb: true,
          thumbPath: dir
        };
    ...
    var setShutter = function(device) {
      ...
      if (!device.metaData.camSettings.shutterOptions.length) {
        ...
        device.metaData.camSettings.shutterOptions = _.remove(device.metaData.camSettings.shutterOptions, (shutterOption) =>{
    ...
    var setAperture = function(device) {
      ...
      if (!device.metaData.camSettings.apertureOptions.length) {
        ...
        device.metaData.camSettings.apertureOptions = _.remove(device.metaData.camSettings.apertureOptions, (apertureOption) =>{
    ...
    var setIso = function(device) {
      ...
      if (!device.metaData.camSettings.isoOptions.length) {
        ...
        //remove AUTO from the Array if it's there
        device.metaData.camSettings.isoOptions = _.remove(device.metaData.camSettings.isoOptions, (isoOption) =>{
    ...
    function handleTimelapseReconnect(pulse, state){
      ...
      $cordovaNativeStorage.getItem('timelapse').then((savedTl) => {
    ...
    function handleVideoReconnect(pulse){
      $cordovaNativeStorage.getItem('video').then((savedVideo) => {
    ...
    function handleBulbReconnect(pulse){
      $cordovaNativeStorage.getItem('bulb').then((savedBulb) => {
    ...
    return {
      ...
      increaseMTU: function(device) {
        ...
        if ($platform.isAndroid()) {
          ...
          ble.increaseMTU(device.id, 158, (mtu) => {
      ...
      resetMetaData: function(device, cameraConnected = false, fullRefresh = true) {
      ...
      scanAndConnectDevices: function(timeout, deferred, isManual = false) {
        ...
        this.handleOutOfSyncDevices().then(() => {
          ble.isEnabled(() => {
            ...
            ble.stopScan();
            this.scanAndConnect(timeout, isManual).then((result) => {
      ...
      scanAndConnect: function(timeout, isManual = false) {
        ...
        BLE.scan(scanArray, timeout).then((peripheral) => {
          ...
          if (peripheral && peripheral.name && peripheral.name == 'Pulse Bootloader') {
            ...
            this.buildAndConnect(pulseDevices[0], counter, timeout, isManual).then((response) => {
              deferred.resolve(response);
            });
       ...
       */
      buildAndConnect: function(device, counter, timeout, isManual, connectAnyway = true) {
        var deferred = $q.defer();
        ...
        this.fetchDeviceFromLocalStorage(pulse).then((localStorageInfo) => {
          ...
          BLE.connect(pulse).then((connectedDevice) => {
      ...
      postSubscribe: function(pulse) {
        {
          ...
          if (!setPD) {
            ...
            $timeout(() => {
              this.checkForCameraConnection(pulse).then((result) => {
            ...
            _.forEach(connectedPulses, (pulse) => {
              if (pulse.id == device.id) {
                if (deepInject) {
                  ...
                  device.nickname = nickname;
                  $cordovaNativeStorage.getItem('devices').then((localDevices) => {
          ...
          case $config.communication.GET_CAM_STATUS:
            {
              ...
              if (connected == 1) {
                ...
                if (device.metaData.cameraConnected && connected === 0) { // had a disconnecion
                  // Start scan process
                  device.metaData.newSession = true;
                  device.cameraScanComplete = false;
                  device = this.resetMetaData(device);
                  ...
                  $timeout(() => {
                    ...
                    btClassic.isConnected(device.metaData.macAddress).then((response) => {
                      ...
                      console.log('we are are not connected, try to connect');
                       btClassic.connect(device.metaData.macAddress, device).then((result) => {
                        device.btClassic.enabled = true;
                        device.btClassic.connected = true;
                        this.setDevice(device);
          ...
          case $config.communication.GET_CLASSIC_MAC:
            {
              ...
              this.fetchDeviceFromLocalStorage(device).then((localStorageInfo) => {
                ...
                if(os === "11.2.5"){
                  ...
                  btClassic.isConnected(device.metaData.macAddress).then((result) => {
                    ...
                    btClassic.connect(device.metaData.macAddress, device).then((result) => {
                      if ($platform.isAndroid()) {
                        ...
                        btClassic.disconnect(device.metaData.macAddress).then((result2) => {
                          btClassic.connect(device.metaData.macAddress, device).then((result3) => {
          ...
          case $config.communication.GET_PERSISTENT_DATA:
            {
              ...
              if (currentPacket == expectedPackets - 1) {
                ...
                $cordovaNativeStorage.setItem('persistentData', persistentData).then(() => {
      ...
      changeAperture: function(buffer, device) {
        ...
        _.forEach(device.metaData.camSettings.apertureOptions, (option, $index) => {
      ...
      findShutterIndex: function(device, shutterValue, shutterOptions, update = true) {
        var data;
