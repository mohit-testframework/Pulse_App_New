  ...
  pulse.services.factory('btClassic', function ($q, $rootScope, $transmit, $timeout, $config, $cordovaNativeStorage, $platform) {
    ...
    return {
      ...
      read: function read(address) {
        ...
        BluetoothClassic.read(address, function (buffer) {
      ...
      disconnect: function disconnect(address) {
        ...
        BluetoothClassic.disconnect(address, function () {
      ...
      connect: function connect(address, device, deferred) {
        console.log('inside BTClassic connect');
        var _this = this;

        var toggleAction = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
        var bypassModal = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
        ...
        BluetoothClassic.connect(address, function (result) {
          ...
          if (typeof error == 'string' && error.includes('show picker')) {
            ...
            var modalData = {
              ...
              checkboxAction: function checkboxAction() {
                $cordovaNativeStorage.getItem('shouldWarnPicker').then(function (status) {
                  $cordovaNativeStorage.setItem('shouldWarnPicker', !status);
                });
              },
              assignedClass: 'btClassicAlert',
              onButtonClick: function onButtonClick() {
                BluetoothClassic.showPicker(function (result) {
                  deferred.resolve(result);
                  $rootScope.$broadcast('firmwareModalTime', {});
                  $rootScope.$broadcast('btConnect', { success: true });
                }, function (error) {
            ...
            $cordovaNativeStorage.getItem('shouldWarnPicker').then(function (shouldWarnPicker) {
              if (shouldWarnPicker) {
                $rootScope.$broadcast('openModal', modalData);
              } else {
                $timeout(function () {
      ...
      isConnected: function isConnected(address) {
        ...
        BluetoothClassic.isConnected(address, function (response) {
          deferred.resolve(response);
        }, function (error) {
          deferred.reject(error);
        });
      ...
      closeSession: function closeSession() {
        ...
        BluetoothClassic.closeSession(function (result) {
