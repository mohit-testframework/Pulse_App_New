...
(function () {
  ...
  pulse.services.factory('$firmware', function ($q, $cordovaFileTransfer, $cordovaNativeStorage, $cordovaFile, $transmit, $timeout, $views, $config, BLE, $platform, $location, $rootScope, btClassic, $ionicLoading) {
    ...
    return {
      ...
      getCompatability: function getCompatability() {
        ...
        if (!window.cordova) {
          ...
          var url = 'https://s3-us-west-1.amazonaws.com/alpine-firmware/pulse-firmware/menu-compatability.json';
          var targetPath;
          if ($platform.isAndroid()) {
            targetPath = cordova.file.externalApplicationStorageDirectory + 'compatability.json';
          } else {
          ...
          $cordovaFileTransfer.download(url, targetPath, options, trustHosts).then(function (result) {
            ...
            $cordovaFile.readAsText(path, 'compatability.json').then(function (data) {
      ...
      syncMenuCompatability: function syncMenuCompatability() {
        var deferred = $q.defer();
        this.getCompatability().then(function (compatabilityList) {
          $cordovaNativeStorage.setItem('menu-compatability', compatabilityList);
      ...
      getMostRecentFirmwareVersion: function getMostRecentFirmwareVersion() {
        ...
        if (!window.cordova) {
          ...
          var url = 'https://s3-us-west-1.amazonaws.com/alpine-firmware/pulse-firmware/' + appVersion + '/firmware-version-fota.json';
          var targetPath;
          if ($platform.isAndroid()) {
            targetPath = cordova.file.externalApplicationStorageDirectory + 'firmware.json';
          } else {
          ...
          $cordovaFileTransfer.download(url, targetPath, options, trustHosts).then(function (result) {
            ...
            $cordovaFile.readAsText(path, 'firmware.json').then(function (success) {
              ...
              cordova.getAppVersion.getVersionNumber(function (version) {
                ...
                appVersion = version;
                   console.log('Version is ' + version);    
                      ...
                      var url;
                      if ($platform.isAndroid()) {
                        url = 'https://s3-us-west-1.amazonaws.com/alpine-firmware/pulse-firmware/' + appVersion + '/pulse-firmware-dual.bin';
                        targetPath = cordova.file.externalApplicationStorageDirectory + 'firmware.bin';
                      } else {
                        url = 'https://s3-us-west-1.amazonaws.com/alpine-firmware/pulse-firmware/' + appVersion + '/pulse-firmware-ios.bin';
                      ...
                      $cordovaFileTransfer.download(url, targetPath, options, trustHosts).then(function (result) {
                        ...
                        console.log('Firmware binary retrieval failed. Trying local fallback : ' + JSON.stringify(err));
                        var localPath;
                        if ($platform.isAndroid()) {
                          localPath = cordova.file.applicationDirectory + 'www/firmware/pulse-firmware-dual.bin';
                        } else {
                          localPath = cordova.file.applicationDirectory + 'www/firmware/pulse-firmware-ios.bin';
                        }
                        $cordovaFileTransfer.download(localPath, targetPath, options, trustHosts).then(function (result) {
                          console.log('fetched local binary copy');
                          deferred.resolve(result);
                        }, function (error2) {
                          console.log('failed to fetch local binary copy : ' + JSON.stringify(error2));
      ...
      readUpdateFirmware: function readUpdateFirmware() {
        ...
        var deferred = $q.defer();
        this.getMostRecentFirmware().then(function (firmwareFile) {
          _this.readFirmwareFile(firmwareFile).then(function (arrayBuffer) {
          ...
          console.log('failed to download firmware bin file. trying local copy');
        });
        return deferred.promise;
      },
      ...
      readFirmwareFile: function readFirmwareFile(firmwareFile) {
        ...
        $cordovaFile.readAsArrayBuffer(path, firmwareFile.name).then(function (arrayBuffer) {
          deferred.resolve(arrayBuffer);
        }, function (error) {
      ...
      connectToBL: function connectToBL(device) {
        ...
        if (proceed) {
          ...
          var scanForBootloader = function scanForBootloader() {
            ...
            $timeout(function () {
              ...
              BLE.stopScan().then(function () {
                ...
                ble.startScan(scanArray, function (peripheral) {
                    ...
                    BLE.stopScan().then(function () {
                      ...
                      var increaseMTU = function increaseMTU(device) {
                        ...
                        if ($platform.isAndroid()) {
                          ...
                          ble.increaseMTU(device.id, 158, function (mtu) {
                      ...
                      console.log("trying to connect to ID: " + peripheral.id);
                      console.dir(peripheral);
                      ble.connect(peripheral.id, function (dev) {
                        console.log("Connected to bootloader... increasing MTU");
                        BLE.stopScan().then(function () {
                        ...
                        console.log('disconnected from pulse bootloader device');
                        if (peripheral.callbacks && peripheral.callbacks.disconnectCallback) {
                      ...
                      console.log('we cant stop the scan. Lets start everything over again');
                      deferred.reject(error);
                    });
                  }
                }, function (error) {
                  console.log("Error scanning for bootloader. Rejecting promise.");
                  deferred.reject(error);
                });
              });
            }, 2000);
          ...
          BLE.stopScan().then(function () {
            ...
            var that = _this2;
            if ($platform.isAndroid()) {
              console.log("Platform is Android. Need to clear the BT cache to find the Bootloader...");
              $timeout(function () {
                BluetoothClassic.clearCache(function () {
                  $timeout(function () {
      ...
      update: function update(dataArray, device) {
        settings.finalPacketIndex = Math.ceil(dataArray.length / BOOTLOADER_PACKET_SIZE); //get our final packet index
        ...
        var saveYourFOTA = function saveYourFOTA() {
          ...
          ble.read(device.id, $config.bootloader.BL_SERVICE, $config.bootloader.CMD_CHANNEL, bootloaderRX, function (error) {
            console.log("Failed to read blRX: " + error);
          });
        };

        var setupWrite1 = function setupWrite1() {
          write1_pkt = new Uint8Array($views.numToArray4($views.decToHex(dataArray.length)));
          console.log("Final Packet Index: " + settings.finalPacketIndex + " . Data Length: " + dataArray.length);
        };
        ...
        var fwCommandTransmit = function fwCommandTransmit(cmd) {
          ble.write(device.id, $config.bootloader.BL_SERVICE, $config.bootloader.CMD_CHANNEL, $views.numToArray2(cmd), function (res) {}, tranError);
        };

        // Transmit firmware binary packet
        var fwDataTransmit = function fwDataTransmit(packetNum) {
          var array = new Uint8Array(new ArrayBuffer(BOOTLOADER_PACKET_SIZE + 2));
          ...
          if (packetNum == 0xFFFF) {
            ...
            console.log("Loading initial write command with transfer length & MTU into packet.");
            for (i = 0; i < 4; i++) {
          ...
          } else if ((packetNum - 1) * BOOTLOADER_PACKET_SIZE <= dataArray.length) {
            ...
            for (i = 0; i < BOOTLOADER_PACKET_SIZE; i++) {
              if (packetNum * BOOTLOADER_PACKET_SIZE + i >= dataArray.length) {
                ...
                array[i + 2] = $views.decToHex(dataArray[packetNum * BOOTLOADER_PACKET_SIZE + i]);
              }
            }
          } else {
          ...
          ble.write(device.id, $config.bootloader.BL_SERVICE, $config.bootloader.DATA_CHANNEL, array.buffer, function (res) {}, tranError);
        };
        ...
        var kickOff = function kickOff() {
          ...
          ble.startNotification(device.id, $config.bootloader.BL_SERVICE, $config.bootloader.CMD_CHANNEL, bootloaderRX, function (error) {});
